...
public class SolrServiceImpl implements SearchService, IndexingService {
    ...
    @Autowired
    protected IndexObjectServiceFactory indexObjectServiceFactory;
    @Autowired
    ...
    public void indexContent(Context context, IndexableObject indexableObject,
        ...
        try {
            final IndexableObjectService indexableObjectService = indexObjectServiceFactory.
                    getIndexableObjectService(indexableObject);
            if (force || requiresIndexing(indexableObject.getUniqueIndexID(), indexableObject.getLastModified())) {
                final SolrInputDocument solrInputDocument = indexableObjectService.buildDocument(context,
                        indexableObject);
    ...
    public void unIndexContent(Context context, IndexableObject indexableObject, boolean commit)
        ...
        try {
            ...
            indexObjectServiceFactory.getIndexableObjectService(indexableObject).delete(indexableObject);
            if (commit) {
    ...
    public void unIndexContent(Context context, String searchUniqueID, boolean commit)
        ...
        try {
            if (solrSearchCore.getSolr() != null) {
                indexObjectServiceFactory.getIndexableObjectService(searchUniqueID).delete(searchUniqueID);
                if (commit) {
                    solrSearchCore.getSolr().commit();
                }
            }
    ...
    public void updateIndex(Context context, boolean force) {
        try {
            final List<IndexableObjectService> indexableObjectServices = indexObjectServiceFactory.
                    getIndexableObjectServices();
            for (IndexableObjectService indexableObjectService : indexableObjectServices) {
    ...
    public DiscoverResult search(Context context, IndexableObject dso, DiscoverQuery discoveryQuery,
        ...
        if (dso != null) {
            if (dso instanceof Community) {
                ...
                discoveryQuery.addFilterQueries(SearchUtils.RESOURCE_UNIQUE_ID + ":" + new IndexableItem((Item) dso).
                        getUniqueIndexID());
            }
        }
    ...
    protected IndexableObject findIndexableObject(Context context, SolrDocument doc) throws SQLException {
        ...
        final IndexableObjectService indexableObjectService = indexObjectServiceFactory.
                getIndexableObjectServiceByType(type);
        IndexableObject indexableObject = indexableObjectService.findIndexableObject(context, id);
        ...
        try {
            ...
            if (mltResults != null && mltResults.get(item.getType() + "-" + item.getID()) != null) {
                ...
                for (Object relatedDoc : relatedDocs) {
                    ...
                    if (StringUtils.equals(relatedItem.getType(), String.valueOf(Constants.ITEM))) {
