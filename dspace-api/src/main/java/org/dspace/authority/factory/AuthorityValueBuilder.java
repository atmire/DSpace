...
public abstract class AuthorityValueBuilder<T extends AuthorityValue>
{
    ...
    public T buildAuthorityValue(SolrDocument document)
    {
        ...
        if(document.getFieldValue("verified") != null) {
            authorityValue.setVerified(((Boolean) document.getFieldValue("verified")));
        }

        Collection<Object> ids = document.getFieldValues("not_duplicate_of_authority_id");
        if(CollectionUtils.isNotEmpty(ids)) {
            for (Object id : ids) {
                authorityValue.addNotDuplicateOfAuthorityId((String) id);
            }
        }
