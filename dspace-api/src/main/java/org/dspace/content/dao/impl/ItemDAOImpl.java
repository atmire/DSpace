...
public class ItemDAOImpl extends AbstractHibernateDSODAO<Item> implements ItemDAO
{
    ...
    @Override
    public Iterator<Item> findByAuthorityValue(Context context, MetadataField metadataField, String authority, boolean archivedOnly) throws SQLException {
        Query query = null;

        if(archivedOnly){
            query = createQuery(context, "SELECT item FROM Item as item join item.metadata metadatavalue WHERE item.inArchive=:in_archive AND metadatavalue.metadataField = :metadata_field AND metadatavalue.authority = :authority");
            query.setParameter("in_archive", true);
        }
        else {
            query = createQuery(context, "SELECT item FROM Item as item join item.metadata metadatavalue WHERE metadatavalue.metadataField = :metadata_field AND metadatavalue.authority = :authority");
        }
